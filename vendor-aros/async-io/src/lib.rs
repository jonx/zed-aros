...
// AROS is not `cfg(unix)` but has a fd-based reactor: pull the fd traits from
// `std::os::fd`. The Unix-domain-socket types stay unix-only (their impls below
// are `cfg(unix)` and excluded here).
#[cfg(target_os = "aros")]
...
#[inline]
fn set_nonblocking(
    #[cfg(any(unix, target_os = "aros"))] fd: BorrowedFd<'_>,
