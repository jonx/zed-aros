...
// AROS (aros-aarch64 vendored fork): psm ships aarch64 asm for every OS, so the
// upstream `psm_stack_manipulation!` selects the `yes` arm, which needs a libc
// mmap backend that AROS lacks. AROS has no reliable way to detect the stack
// limit or move to a fresh mapped stack, so run the callback in place (no
// growth). Moderate UI recursion depth stays within the default process stack.
#[cfg(target_os = "aros")]
