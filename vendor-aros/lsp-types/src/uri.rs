...
impl Uri {
    ...
    pub fn from_file_path<P: AsRef<std::path::Path>>(path: P) -> Result<Self, ()> {
        #[cfg(target_os = "aros")]
        {
            // url's path<->URL conversion is cfg(unix/windows)-gated and absent
            // on AROS, so build the file:// URL by hand. AROS `MacRW:` volume
            // paths first map to the real host path a host language server can
            // read; the result is a normal POSIX-absolute path.
            let raw = path.as_ref().to_string_lossy();
            let posix = aros_path::to_host(&raw).unwrap_or_else(|| raw.to_string());
            return aros_path::file_url(&posix).map(Self).ok_or(());
        }
    ...
    ///
    /// Inherent method (takes priority over the `Deref` to `url::Url`), so every
    /// caller of `uri.to_file_path()` gets AROS translation for free.
    pub fn to_file_path(&self) -> Result<std::path::PathBuf, ()> {
        #[cfg(target_os = "aros")]
        {
            // url::Url::to_file_path is cfg(unix/windows)-gated (absent on AROS);
            // decode the path segments manually, then map the host path back to
            // the AROS host-shared volume.
            let host = aros_path::url_to_host(&self.0).ok_or(())?;
            let aros = aros_path::to_aros(&host).unwrap_or(host);
            return Ok(std::path::PathBuf::from(aros));
        }
        #[cfg(not(target_os = "aros"))]
        {
            self.0.to_file_path()
...
/// AROS <-> host path translation for the LSP host bridge. The AROS
/// host-shared volume (`MacRW:`) is a real host directory; a language server
/// running on the host needs those host paths, and its responses come back as
/// host paths that must map back to AROS volume paths for the editor.
#[cfg(target_os = "aros")]
mod aros_path {
    /// The AROS volume that maps to the host-shared directory.
    const VOLUME: &str = "MacRW:";

    /// The host directory `MacRW:` resolves to. Overridable so the mapping is
    /// not machine-specific; must match the aros-ctl host-volume mount.
    fn host_root() -> String {
        std::env::var("ZED_AROS_HOST_SHARED")
            .unwrap_or_else(|_| "/Users/jkn/AROS/Shared".to_string())
    }

    /// `MacRW:proj/src/main.rs` -> `/Users/jkn/AROS/Shared/proj/src/main.rs`.
    /// Returns None for paths outside the shared volume (left untranslated).
    pub fn to_host(path: &str) -> Option<String> {
        let rest = path.strip_prefix(VOLUME)?;
        let root = host_root();
        let sep = if root.ends_with('/') { "" } else { "/" };
        Some(format!("{root}{sep}{rest}"))
    }

    /// The reverse: `/Users/jkn/AROS/Shared/proj/src/main.rs` -> `MacRW:proj/...`.
    pub fn to_aros(path: &str) -> Option<String> {
        let root = host_root();
        let root = root.strip_suffix('/').unwrap_or(&root);
        let rest = path.strip_prefix(root)?.trim_start_matches('/');
        Some(format!("{VOLUME}{rest}"))
    }

    /// Characters escaped inside a path segment of a file:// URL (mirrors url's
    /// SPECIAL_PATH_SEGMENT closely enough for language-server paths).
    ...
    /// Build a `file://` URL from a POSIX-absolute path (segment-encoded).
    pub fn file_url(posix_abs: &str) -> Option<url::Url> {
        if !posix_abs.starts_with('/') {
            return None;
        }
        let mut s = String::from("file://");
        for seg in posix_abs.split('/').skip(1) {
            s.push('/');
            s.extend(percent_encoding::utf8_percent_encode(seg, SEGMENT));
        }
        if s == "file://" {
            s.push('/');
        }
    ...
    /// Decode a file:// URL back to its POSIX host path (segment percent-decode).
    pub fn url_to_host(url: &url::Url) -> Option<String> {
        let segments = url.path_segments()?;
        let mut host = String::new();
        for seg in segments {
            host.push('/');
            let decoded = percent_encoding::percent_decode(seg.as_bytes()).decode_utf8_lossy();
            host.push_str(&decoded);
        }
        if host.is_empty() {
            host.push('/');
        }
