...
// AROS is not `cfg(unix)` and has no getpwuid; HOME env only.
#[cfg(target_os = "aros")]
mod target_aros {
    ...
    pub fn user_dir(_user_dir_name: &str) -> Option<PathBuf> { None }
    pub fn user_dirs(_home_dir_path: &Path) -> HashMap<String, PathBuf> { HashMap::new() }
}
#[cfg(target_os = "aros")]
