...
#[cfg(all(feature = "std", any(unix, windows, target_os = "aros")))]
variant_identifier! {
...
impl<'de> Visitor<'de> for OsStringVisitor {
    ...
    // AROS OS strings are byte-based like Unix; accept the "Unix" variant.
    #[cfg(target_os = "aros")]
    fn visit_enum<A>(self, data: A) -> Result<Self::Value, A::Error>
    where
        A: EnumAccess<'de>,
    {
        use std::os::aros::ffi::OsStringExt;

        match tri!(data.variant()) {
            (OsStringKind::Unix, v) => v.newtype_variant().map(OsString::from_vec),
            (OsStringKind::Windows, _) => Err(Error::custom(
                "cannot deserialize Windows OS string on AROS",
            )),
        }
    }
...
#[cfg(all(feature = "std", any(unix, windows, target_os = "aros")))]
#[cfg_attr(docsrs, doc(cfg(all(feature = "std", any(unix, windows, target_os = "aros")))))]
