...
// AROS is not `cfg(unix)` and has no getpwuid; use the HOME env only. XDG user
// dirs aren't a concept there, so user_dir/user_dirs are inert.
#[cfg(target_os = "aros")]
mod target_aros {
    ...
    pub fn user_dir(_user_dir_name: &str) -> Option<PathBuf> {
        None
    }

    pub fn user_dirs(_home_dir_path: &Path) -> HashMap<String, PathBuf> {
        HashMap::new()
    }
}

#[cfg(target_os = "aros")]
pub use self::target_aros::{home_dir, user_dir, user_dirs};
